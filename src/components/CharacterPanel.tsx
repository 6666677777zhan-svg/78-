...
import { 
  ...
  Sun, Moon, Skull, PlusCircle, Check, Camera, Image, BookOpen, Layers, Dices, Cloud, ShieldCheck
} from 'lucide-react';
    ...
    <div className="space-y-6 relative">
      ...
      <div className="bg-slate-900/90 border border-amber-500/40 rounded-3xl p-6 shadow-2xl relative overflow-hidden">
        ...
        <div className="relative z-10 flex flex-col md:flex-row items-center justify-between gap-6">
          ...
          {/* Google / Gmail Cloud Save Status Bar */}
          {onOpenGoogleCloud && (
            <div className="bg-gradient-to-r from-blue-950/40 via-slate-950 to-indigo-950/40 border border-blue-500/30 p-3 rounded-2xl flex flex-col sm:flex-row items-center justify-between gap-3 shadow-md">
              <div className="flex items-center gap-2.5">
                ...
                <div>
                  <span className="text-xs font-bold text-slate-200 block">
                    {activeGoogleUser ? `Google 账号: ${activeGoogleUser.email}` : 'Google / Gmail 云端存档: 未绑定 (游客模式)'}
                  </span>
                  <span className="text-[10px] text-slate-400">
                    {activeGoogleUser ? '云端已连接 · 每次突破与获得魂环自动保存' : '绑定 Google 账号可将全部魂环魂骨跨设备持久保存'}
                  </span>
                </div>
              </div>
              ...
              <button
                ...
                className="px-3 py-1.5 bg-gradient-to-r from-blue-600 to-indigo-600 hover:from-blue-500 hover:to-indigo-500 text-white rounded-xl text-xs font-black flex items-center gap-1 shadow-[0_0_12px_rgba(59,130,246,0.3)] transition-all active:scale-95 whitespace-nowrap"
              >
                <Cloud className="w-3.5 h-3.5" />
                <span>{activeGoogleUser ? '管理 Google 云端存档' : '使用 Google 登录 / 存到云端'}</span>
              </button>
            </div>
          )}
