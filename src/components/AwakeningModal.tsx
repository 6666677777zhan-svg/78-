...
import { 
  ...
  Check, Image, Volume2, Dices, Shuffle, Cloud, Mail
} from 'lucide-react';
      ...
      <div className="bg-slate-900/90 border-2 border-amber-500/50 rounded-3xl max-w-4xl w-full p-6 md:p-8 text-slate-100 shadow-[0_0_80px_rgba(245,158,11,0.25)] relative overflow-hidden flex flex-col my-auto">
        ...
        {step === 'hall' && (
          <div className="space-y-6 text-center">
            ...
            {/* Google / Gmail Cloud Account Banner */}
            <div className="bg-gradient-to-r from-blue-950/40 via-slate-900 to-indigo-950/40 border border-blue-500/30 rounded-2xl p-3.5 flex flex-col sm:flex-row items-center justify-between gap-3">
                ...
                <div>
                  <div className="flex items-center gap-1.5">
                    <span className="text-xs font-bold text-slate-200">
                      {activeGoogleUser ? `已连接 Google: ${activeGoogleUser.email}` : 'Google / Gmail 云端存档'}
                    </span>
                    {activeGoogleUser && (
                  ...
                  <span className="text-[10px] text-slate-400">
                    {activeGoogleUser 
                      ? '觉醒后将自动云端同步至您的 Google 账户' 
                      : '登录 Google / Gmail 可将存档永久保存在云端并跨设备同步'}
                  </span>
                </div>
              </div>
              ...
              {onOpenGoogleCloud && (
                <button
                  ...
                  className="px-3 py-1.5 bg-blue-600 hover:bg-blue-500 text-white rounded-xl text-xs font-bold flex items-center gap-1.5 transition-all shadow-[0_0_12px_rgba(59,130,246,0.3)] active:scale-95 whitespace-nowrap"
                >
                  <Cloud className="w-3.5 h-3.5" />
                  <span>{activeGoogleUser ? '管理云存档' : 'Google / Gmail 登录'}</span>
                </button>
              )}
            </div>
